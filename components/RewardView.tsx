...
import { Home, Sparkles, Loader2, Share2, Facebook, Star, Info } from 'lucide-react';
...
export const RewardView: React.FC<RewardViewProps> = ({ onReset, capturedImage }) => {
  ...
  const handleShareWithPhoto = async () => {
    try {
      ...
      // Per includere la FOTO scattata, dobbiamo usare navigator.share.
      // Il metodo "link" (URL sharer) non può inviare file locali dal dispositivo.
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Rocca Fun Factory Challenge',
          text: `Ho vinto il Golden Balloon Dog! 🎈🐶 ${tags}`,
        });
      } else {
        // Fallback se il browser non supporta la condivisione file
        const link = document.createElement('a');
        link.href = capturedImage;
        link.download = 'rocca-fun-factory-photo.jpg';
        link.click();
        alert("La condivisione diretta della foto non è supportata su questo browser. Abbiamo scaricato la foto per te: ora puoi caricarla manualmente su Facebook incollando i tag!");
      }
    } catch (err) {
      ...
      <div className="z-10 animate-bounceIn w-full flex flex-col items-center max-w-sm">
        ...
        <div className="bg-blue-50 border-2 border-blue-100 p-5 rounded-[1.5rem] w-full mb-6 flex items-start space-x-3 text-left">
          ...
          <p className="font-bold text-[11px] leading-relaxed text-blue-900 uppercase tracking-tight">
            Per ricevere il premio, usa il tasto sotto, seleziona Facebook e pubblica la foto con i tag!
          </p>
        </div>

        {/* Pulsante Unificato per Condivisione REALE della foto */}
        <div className="w-full space-y-3">
          <button 
            onClick={handleShareWithPhoto}
            className="w-full bg-[#1877F2] text-white font-black py-5 rounded-2xl shadow-xl flex items-center justify-center space-x-4 active:scale-95 transition-all uppercase tracking-widest text-base"
          >
